...
async function getCredentials(): Promise<[string, string]> {
  ...
  if (argv.env !== undefined) {
    ...
    if (envFound.error) {
      console.error(
        `Specified --env option but cannot load .env (${envFound.error})`
      );
      process.exit(1);
...
async function zip(output: string): Promise<string> {
  ...
  if (
    !fs.existsSync(argv.path as string) ||
    !fs.lstatSync(argv.path as string).isDirectory()
  ) {
    console.error(
      `The target path "${argv.path as string}" is not a directory`
    );
    process.exit(0);
  }
  ...
  const rbzPath = `${path.join(
    path.dirname(output),
    path.parse(output).name
  )}_not_signed.rbz`;
...
async function sign(
  ...
  try {
    await download(downloadUrl, path.dirname(outputPath), {
